...
// ---- type scale v2 (2026-09-01, Adam: "原本的字太小了", second time) --------
// The first scale went 12.5 → 16 after the same note. It was still too small,
// and the reason is that cutting words twice buys type twice: the page now
// holds four or five rows, not eight, so 18 pt fits where 16 did.
// Nothing on a content page is under 12.
const T = {
  title: 36,      // page head, all caps
  kicker: 15,     // fragments under it — never a sentence
  body: 18,       // the value half of a row: the page's main text
  label: 16,      // the label half
  q: 22,          // the QUESTION line
  qsub: 14.5,
  table: 15,
  chip: 13,
  key: 16,        // the one takeaway line above the footnote
  foot: 12,       // provenance — the smallest thing allowed on a page
};
...
function head(s, title, kicker) {
  s.addText(title.toUpperCase(), {
    x: M, y: 0.60, w: CW, h: 0.70,
    fontFace: FH, fontSize: T.title, bold: true, color: INK, margin: 0, charSpacing: 0.4,
  });
  if (kicker) {
    s.addText(kicker, {
      x: M, y: 1.36, w: CW, h: 0.32,
      fontFace: FB, fontSize: T.kicker, color: FAINT, margin: 0, charSpacing: 0.3,
    });
  }
  s.addShape("line", { x: M, y: kicker ? 1.82 : 1.46, w: CW, h: 0.01,
    line: { color: RULE, width: 1 } });
}

// A page whose content is four or five numbers, set large enough to read from
// the back of the room. Used when the slide is a springboard rather than the
// evidence — the 9/03 manual section, where the professor is shown the live
// website and the slide only has to carry the headline counts.
//
// `stats` is [[number, caption], …]. Two rows of up to three.
function bigStats(s, stats, y0) {
  const per = Math.ceil(stats.length / (stats.length > 3 ? 2 : 1));
  const colW = (CW - (per - 1) * 0.4) / per;
  stats.forEach((st, i) => {
    const [n, cap, colour] = st;
    const col = i % per, rowI = Math.floor(i / per);
    const x = M + col * (colW + 0.4);
    const y = (y0 || 2.20) + rowI * 2.15;
    s.addText(n, {
      x, y, w: colW, h: 1.15, valign: "middle",
      fontFace: FH, fontSize: 68, bold: true, color: colour || ACCENT, margin: 0,
    });
    s.addShape("line", { x, y: y + 1.20, w: colW * 0.42, h: 0.01,
      line: { color: RULE, width: 1.5 } });
    s.addText(cap, {
      x, y: y + 1.32, w: colW, h: 0.66, valign: "top",
      fontFace: FB, fontSize: 15.5, color: BODY, margin: 0, lineSpacingMultiple: 1.14,
    });
  });
}
...
function row(s, x, y, w, label, value, o) {
  ...
  const lw = o.lw || 2.30;
  const h = o.h || 0.60;
  s.addShape("rect", { x, y, w: 0.055, h,
    fill: { color: o.tick || ACCENT }, line: { type: "none" } });
  s.addText(label, {
    x: x + 0.24, y, w: lw - 0.24, h, valign: "middle",
    fontFace: FB, fontSize: o.lfs || T.label, bold: true,
    color: o.lc || INK, margin: 0, charSpacing: 0.5,
  });
  s.addText(value, {
    x: x + lw, y, w: w - lw, h, valign: "middle",
    fontFace: FB, fontSize: o.fs || T.body, color: o.vc || BODY, margin: 0,
...
function chip(s, x, y, w, text, color) {
  ...
  s.addShape("rect", { x, y, w, h: 0.40, fill: { color: bg },
    line: { color, width: 1.3 } });
  s.addText(text, { x, y, w, h: 0.40, align: "center", valign: "middle",
    fontFace: FB, fontSize: T.chip, bold: true, color, margin: 0, charSpacing: 0.4 });
}
...
function question(s, x, y, w, q, sub) {
  s.addShape("rect", { x, y, w: 0.055, h: sub ? 0.94 : 0.56,
    fill: { color: ACCENT }, line: { type: "none" } });
  s.addText("QUESTION", { x: x + 0.24, y: y - 0.02, w: w - 0.24, h: 0.30,
    fontFace: FB, fontSize: 12.5, bold: true, color: ACCENT, margin: 0, charSpacing: 0.8 });
  s.addText(q, { x: x + 0.24, y: y + 0.30, w: w - 0.24, h: 0.38,
    fontFace: FB, fontSize: T.q, bold: true, color: INK, margin: 0 });
  if (sub) {
    s.addText(sub, { x: x + 0.24, y: y + 0.70, w: w - 0.24, h: 0.28,
      fontFace: FB, fontSize: T.qsub, color: MUTED, margin: 0 });
  }
}

// Provenance, bottom-left. Commits and file paths live here and nowhere else.
function footNote(s, text) {
  s.addText(text, { x: M, y: 6.92, w: CW - 0.75, h: 0.32,
    fontFace: FB, fontSize: T.foot, color: FAINT, margin: 0 });
}

// One line of takeaway above the footnote. It costs 0.36" of the page, so the
// last row on a page that carries one has to end by 6.42 — measured, twice.
function keyLine(s, text) {
  s.addText(text, { x: M, y: 6.44, w: CW - 0.75, h: 0.34,
    fontFace: FB, fontSize: T.key, bold: true, color: ACCENT, margin: 0 });
}
