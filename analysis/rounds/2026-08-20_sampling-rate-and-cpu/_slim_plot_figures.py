...
def fig_tradeoff(fname):
    ...
    axL.set_title("What you get", fontsize=11.5, color=INK, loc="left", pad=16, weight="bold")
    axL.text(0, 1.018,
    ...
    axR.legend(frameon=False, fontsize=8.6, loc="upper center", bbox_to_anchor=(0.5, -0.145),
               ...
               handlelength=1.2)
    axR.set_title("What it costs", fontsize=11.5, color=INK, loc="left", pad=16, weight="bold")
    ...
    fig.suptitle("Sampling harder buys precision on a √ law"
                 "", fontsize=13, color=INK, x=0.012, y=0.972, ha="left", weight="bold")
...
def fig_where(fname):
    ...
    axB.set_title("The same total, opened up: 3 of 10 switches do the work",
                  fontsize=11.5, color=INK, loc="left", pad=16, weight="bold")
    ...
    fig.suptitle("The sampling bill lands on the kernel and the proxy, not the switches"
                 "", fontsize=13, color=INK, x=0.012, y=0.972, ha="left", weight="bold")
...
def fig_iperf(fname):
    ...
    fig.suptitle("The busiest process on the box is the instrument, not the fabric",
...
def fig_concurrency(fname):
    ...
    fig.suptitle("The northbound API serves one request at a time \u2014 measured, and it has "
                 "room today", fontsize=13, color=INK, x=0.012, y=0.972, ha="left", weight="bold")
...
def fig_decomposition(fname):
    ...
    fig.suptitle("206 µs of CPU per sample, inside the range measured"
                 "", fontsize=13, color=INK, x=0.012, y=0.972, ha="left", weight="bold")
...
def fig_ladder_inherited(fname):
    ...
    for ax, (label, colour, rows) in zip(axes, panels):
        ...
        ax.set_title(label, fontsize=10.5, color=colour, loc="left", pad=16, weight="bold")
    ...
    fig.suptitle("The staircase, and its jitter, predate the fork",
                 fontsize=13.5, color=INK, x=0.012, y=0.972, ha="left", weight="bold")


    fig.tight_layout(rect=[0, 0, 1, 0.935])
    fig.subplots_adjust(top=0.80, wspace=0.09)
    fig.savefig(os.path.join(OUT, fname), dpi=200)
    plt.close(fig)
